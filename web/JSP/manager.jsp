        ...
        <div class="sidebar-content">
            ...
            <div class="card card-sidebar-mobile">
                <ul class="nav nav-sidebar" data-nav-type="accordion">
                    ...
                    <li class="nav-item"><a href="/ManagerApply" class="nav-link active">选课事务管理</a></li>
        ...
        <div class="content pt-0">
            ...
            <div class="d-flex align-items-start flex-column flex-md-row">
                <div class="order-3 order-md-3" style="width: 100%;">
                    <%
                        Map<String, ArrayList<ArrayList<String>>> applyList = (Map<String, ArrayList<ArrayList<String>>>)request.getAttribute("applyList");
                        if(applyList!=null && applyList.size() != 0){
                            Set<String> apply_list = applyList.keySet();
                            for (String name:apply_list) {
                    %>
                    <div class="card">
                            ...
                            <div class="mb-4">
                                <div class="table-responsive mb-3">
                                    <table class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                        <%
                                            for (ArrayList<String> a:applyList.get(name)
                                            ) {
                                        %>
                                        <tr>
                                            <td><%=a.get(1)%></td>
                                            <td><%=a.get(0)%></td>
                                            <td><%=a.get(2)%></td>
                                            <td><button type="button" class="btn btn-info" onclick="location='/HandleApply?student_id=<%=a.get(0)%>&section_id=<%=Integer.parseInt(a.get(4))%>&course_id=<%=a.get(3)%>&choose=0'">不通过</button></td>
                                            <td><button type="button" class="btn btn-danger" onclick="location='/HandleApply?student_id=<%=a.get(0)%>&section_id=<%=Integer.parseInt(a.get(4))%>&course_id=<%=a.get(3)%>&choose=1'">通过</button></td>
                                        </tr>
                                        <%
                                            }
                                        %>
